...
public class DatabaseHelper extends SQLiteOpenHelper
{
    ...
    {
        String CREATE_TABLE_CUSTOMER = "CREATE TABLE IF NOT EXISTS " + this.TABLE_CUSTOMER +
                " (cid TEXT, " +
                "first_name TEXT, " +
                "surname TEXT, " +
                "email TEXT, " +
        ...
        String CREATE_TABLE_APP_SETTING = "CREATE TABLE IF NOT EXISTS " + this.TABLE_APP_SETTING +
                " (id TEXT," +
                "value TEXT," +
                "PRIMARY KEY(id))";

        String CREATE_TABLE_PAIR = "CREATE TABLE IF NOT EXISTS " + this.TABLE_PAIR +
                " (_id INTEGER PRIMARY KEY, " +
                "customer_id TEXT, " +
                "shirt_path TEXT, " +
                "pant_path TEXT)";

        String CREATE_TABLE_PAIR_HISTORY = "CREATE TABLE IF NOT EXISTS " + this.TABLE_PAIR_HISTORY +
                " (_id INTEGER PRIMARY KEY, " +
                "customer_id TEXT, " +
                "shirt_path TEXT, " +
                "pant_path TEXT)";
    ...
    public Customer authenticateCustomer(String email, String password)
    {
        ...
        try
        {
            ...
            this.m_openDatabase(db);
            String sql = "SELECT * FROM " + this.TABLE_CUSTOMER
            ...
            if (cursor.moveToFirst())
            {
                do
                {
                    ...
                    customer.setCustomerID(cursor.getString(cursor.getColumnIndex("cid")).toLowerCase());
                    customer.setFirstName(cursor.getString(cursor.getColumnIndex("first_name")).toLowerCase());
                    customer.setSurname(cursor.getString(cursor.getColumnIndex("surname")).toLowerCase());
                    customer.setEmail(cursor.getString(cursor.getColumnIndex("email")).toLowerCase());
                    customer.setPassword(cursor.getString(cursor.getColumnIndex("password")).toLowerCase());
                    customer.setDob(cursor.getString(cursor.getColumnIndex("dob")).toLowerCase());
                    customer.setMobileNumber(cursor.getString(cursor.getColumnIndex("contact_no")).toLowerCase());
                }
    ...
    private void m_openDatabase(SQLiteDatabase db)
    {
        try
        {
            if (!db.isOpen())
            {
                SQLiteDatabase.openDatabase(DATABASE_NAME, null, DATABASE_VERSION);
            }
        }
        catch (SQLiteException e)
        {
    ...
    private Customer m_getCustomer(String customerID)
    {
        ...
        try
        {
            ...
            String sql = "SELECT * FROM "
                    + this.TABLE_CUSTOMER
                    + " WHERE cid='" + customerID.toLowerCase() + "'";
            Cursor cursor = db.rawQuery(sql, null);
            if (cursor.moveToFirst())
            {
                do
    ...
    public void savePair(Pair pair)
    {
        try
        {
            if (pair != null)
            {
                ...
                pairValues.put("customer_id", pair.getCustomerID().toLowerCase());
    ...
    public void savePairHistory(Pair pair)
    {
        try
        {
            if (pair != null)
            {
                ...
                pairHistoryValues.put("customer_id", pair.getCustomerID().toLowerCase());
    ...
    public ArrayList<Pair> getPairs(String source)
    {
        ...
        try
        {
            ...
            String sql = "SELECT * FROM " + tableName
                    + " WHERE customer_id='"
                    + AppModel.customer.getCustomerID().toLowerCase() + "'";
            Cursor cursor = db.rawQuery(sql, null);
            if (cursor.moveToFirst())
            {
                do
                {
                    Pair pair = new Pair();
                    pair.setCustomerID(cursor.getString(cursor.getColumnIndex("customer_id")).toLowerCase());
