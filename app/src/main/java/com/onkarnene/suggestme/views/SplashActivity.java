...
import com.onkarnene.suggestme.databinding.ActivitySplashBinding;
...
public class SplashActivity extends AppCompatActivity {
	...
	private ActivitySplashBinding m_binding;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		m_binding = ActivitySplashBinding.inflate(getLayoutInflater());
	...
	private class SplashAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			try {
				if (m_checkDataBase()) {
					Customer customer = DatabaseHelper.getInstance(m_context)
					                                  .getLastLoggedInCustomer();
					if (customer != null) {
